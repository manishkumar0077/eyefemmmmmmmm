...
import { Loader2, AlertTriangle, Plus, Save, Trash2, GripVertical, Star, ThumbsUp, FileText, BookOpen, Calendar, Baby, HeartPulse, Stethoscope, Syringe, Microscope, Activity, Shield, Search, Pill, FlaskConical, Users, Hourglass } from 'lucide-react';
import { toast } from 'sonner';
import { useGyneExpertise, GyneExpertise, iconMap, IconName } from '@/hooks/useGyneExpertise';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export const GyneExpertiseEditor = () => {
  const { expertise, isLoading, error, refreshExpertise, updateExpertise, addExpertise, deleteExpertise } = useGyneExpertise();
  const [selectedExpertise, setSelectedExpertise] = useState<GyneExpertise | null>(null);
  const [isAdding, setIsAdding] = useState(false);
  const [newExpertise, setNewExpertise] = useState<Omit<GyneExpertise, 'id' | 'created_at'>>({
    title: '',
    description: '',
    icon: 'Star',
  ...
  const handleSelectExpertise = (expertise: GyneExpertise) => {
